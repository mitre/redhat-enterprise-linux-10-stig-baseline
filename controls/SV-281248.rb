...
The only legitimate location for device files is the "/dev" directory located on the root partition, with the exception of chroot jails if implemented.'
  desc 'check', %q(Verify RHEL 10 is configured so that all nonroot local partitions are mounted with the "nodev" option with the following command:

$ sudo mount | grep '^/dev\S* on /\S' | grep --invert-match 'nodev'

If any output is produced, this is a finding.)
  desc 'fix', 'Configure RHEL 10 so that "/etc/fstab" uses the "nodev" option on all nonroot local partitions.'
  impact 0.5
  tag severity: 'medium'

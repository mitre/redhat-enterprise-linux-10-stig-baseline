...
Authoritative time sources include the United States Naval Observatory (USNO) time servers, a time server designated for the appropriate DOD network (NIPRNet/SIPRNet), and/or the Global Positioning System (GPS).'
  desc 'check', 'Verify RHEL 10 is securely comparing internal information system clocks at least every 24 hours with an NTP server with the following commands:
...
If the parameter "server" is not set or is not set to an authoritative DOD time source, this is a finding.'
  desc 'fix', 'Configure RHEL 10 to securely compare internal information system clocks at least every 24 hours with an NTP server by adding/modifying the following line in the "/etc/chrony.conf" file:
...
server [ntp.server.name] iburst maxpoll 16'
  ...
  tag cci: ['CCI-001891', 'CCI-001890', 'CCI-002046', 'CCI-004923', 'CCI-004926', 'CCI-004922']
  tag nist: ['AU-8 (1) (a)', 'AU-8 b', 'AU-8 (1) (b)', 'SC-45 (1) (a)', 'SC-45 (1) (b)', 'SC-45']
  tag 'host'

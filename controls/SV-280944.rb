...
Removing the "telnet-server" package decreases the risk of accidental (or intentional) activation of the telnet service.'
  desc 'check', 'Verify RHEL 10 does not have the "telnet-server" package installed with the following command:

$ sudo dnf list --installed telnet-server
Error: No matching Packages to list

If the "telnet-server" package is installed, this is a finding.'
  desc 'fix', 'Configure RHEL 10 to not have the "telnet-server" package installed with the following command:

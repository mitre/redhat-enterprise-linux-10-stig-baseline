...
'
  desc 'check', 'Verify RHEL 10 is configured to audit the execution of the "newgrp" command with the following command:
...
If the command does not return a line, or the line is commented out, this is a finding.'
  desc 'fix', 'Configure RHEL 10 to generate audit records upon successful and unsuccessful attempts to use the "newgrp" command by adding or updating the following rule in "/etc/audit/rules.d/audit.rules":
...
$ sudo service auditd restart'
  impact 0.5
  tag check_id: 'C-85694r1166349_chk'
  tag severity: 'medium'
  tag gid: 'V-281133'

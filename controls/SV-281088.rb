...
The only legitimate location for device files is the "/dev" directory located on the root partition, with the exception of chroot jails if implemented.'
  desc 'check', 'Verify RHEL 10 is configured so that "/home" is mounted with the "nodev" option with the following command:
...
If the "/home" file system is mounted without the "nodev" option, this is a finding.'
  desc 'fix', 'Configure RHEL 10 to prevent device files from being interpreted on file systems that contain user home directories.

Modify "/etc/fstab" to use the "nodev" option on the "/home" directory.'
  impact 0.5
  tag check_id: 'C-85649r1165617_chk'
  tag severity: 'medium'
  tag gid: 'V-281088'

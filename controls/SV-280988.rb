...
- x509/name - Certificate validation and subject name authentication.'
  desc 'check', %q(Verify RHEL 10 encrypts audit records offloaded onto a different system via rsyslog using the modern "omfwd" action.
...
If an active TCP-based "omfwd" forwarding rule exists on the system but the command above returns no active configuration lines, or if all results are commented out, this is a finding.)
  desc 'fix', 'Configure RHEL 10 to encrypt off-loaded audit records via rsyslog by enabling TLS within the "omfwd" action rule.

control 'SV-281046' do
  ...
  desc 'If an unauthorized or modified device is allowed to exist on the system, the system may perform unintended or unauthorized operations.'
  desc 'check', 'Verify RHEL 10 is configured so that all system device files are correctly labeled to prevent unauthorized modification.
...
If there is output from either of these commands, other than already noted, this is a finding.'
  desc 'fix', 'Configure RHEL 10 so that all system device files are correctly labeled to prevent unauthorized modification.
...
If a package was not used to install the SELinux policy for a given device class, it must be generated manually and provide specific type labels.'
  impact 0.5
  tag check_id: 'C-85607r1165491_chk'
  tag severity: 'medium'
  tag gid: 'V-281046'

control 'SV-281265' do
  title 'RHEL 10 must not permit direct logins to the root account using remote access via Secure Shell (SSH).'
  desc "Even though the communications channel may be encrypted, an additional layer of security is gained by extending the policy of not logging directly on as root. In addition, logging in with a user-specific account provides individual accountability of actions performed on the system and helps to minimize direct attack attempts on root's password.
...
OpenSSH uses the first occurrence of a keyword it sees, and drop-in files are read in lexicographical order at the start of the configuration. Red Hat recommends using drop-in files rather than changing base configuration files."
  desc 'check', %q(Verify RHEL 10 remote access using SSH prevents users from logging on directly as "root" with the following command:
...
If the "PermitRootLogin" keyword is not set to "no" in a drop-in that lexicographically precedes 50-redhat.conf, or if no output is returned, this is a finding.)
  desc 'fix', 'Configure RHEL 10 to prevent SSH users from logging on directly as root.
...
$ sudo systemctl restart sshd.service'
  impact 0.5
  tag check_id: 'C-85826r1166745_chk'
  tag severity: 'medium'
  tag gid: 'V-281265'
  tag rid: 'SV-281265r1184765_rule'
  tag stig_id: 'RHEL-10-700620'
  tag gtitle: 'SRG-OS-000109-GPOS-00056'
  tag fix_id: 'F-85731r1166746_fix'
  tag 'documentable'
  tag cci: ['CCI-004045']
  tag nist: ['IA-2 (5)']
end

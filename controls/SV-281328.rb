control 'SV-281328' do
  title 'RHEL 10 must require authentication to access single-user mode.'
...
This requirement prevents attackers with physical access from trivially bypassing security on the machine and gaining root access. Such accesses are further prevented by configuring the bootloader password.'
  desc 'check', 'Verify RHEL 10 requires authentication for single-user mode with the following command:
...
Note: The configuration setting can be in either the default location or the drop-in file but not in both locations.'
  desc 'fix', 'Configure RHEL 10 to require authentication for single-user mode.
...
$ sudo systemctl daemon-reload'
  impact 0.5
  tag check_id: 'C-85889r1167132_chk'
  tag severity: 'medium'
  tag gid: 'V-281328'
  tag rid: 'SV-281328r1167134_rule'
  tag stig_id: 'RHEL-10-701260'
  tag gtitle: 'SRG-OS-000080-GPOS-00048'
  tag fix_id: 'F-85794r1167133_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
  tag 'host'

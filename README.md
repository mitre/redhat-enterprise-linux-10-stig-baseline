# RedHat Enterprise Linux 10.x Security Technical Implementation Guide InSpec Profile

The Red Hat Enterprise Linux 10.x Security Technical Implementation Guide (RHEL10.x STIG) InSpec Profile can help programs automate their compliance checks of Red Hat Enterprise Linux 10.x systems to Department of Defense (DoD) requirements.

- Profile Version: `1.2.0`
- RedHat Enterprise Linux 10 Security Technical Implementation Guide v1r2
...
The RHEL10 STIG Profile uses the [InSpec](https://github.com/inspec/inspec) open-source compliance validation language to support automation of the required compliance, security and policy testing for Assessment and Authorization (A&A) and Authority to Operate (ATO) decisions and Continuous Authority to Operate (cATO) processes.

Table of Contents
=================

- [RedHat Enterprise Linux 10.x Security Technical Implementation Guide InSpec Profile](#redhat-enterprise-linux-10x-security-technical-implementation-guide-inspec-profile)
  - [RedHat 10.x Enterprise Linux Security Technical Implementation Guide (RHEL10 STIG)](#redhat-10x-enterprise-linux-security-technical-implementation-guide-rhel10-stig)
...
## RedHat 10.x Enterprise Linux Security Technical Implementation Guide (RHEL10 STIG)
...
The RHEL10 STIG (see public.cyber.mil/stigs/) offers a comprehensive compliance guide for the configuration and operation of your Red Hat Enterprise Linux 10.x system.

The requirements associated with the RHEL10 STIG are derived from the [Security Requirements Guides](https://csrc.nist.gov/glossary/term/security_requirements_guide) and align to the [National Institute of Standards and Technology](https://www.nist.gov/) (NIST) [Special Publication (SP) 800-53](https://csrc.nist.gov/Projects/risk-management/sp800-53-controls/release-search#!/800-53) Security Controls, [DoD Control Correlation Identifier](https://public.cyber.mil/stigs/cci/) and related standards.

The RHEL10.x STIG profile checks were developed to provide technical implementation validation to the defined DoD requirements, the guidance can provide insight for any organizations wishing to enhance their security posture and can be tailored easily for use in your organization.

### Source Guidance

- RedHat Enterprise Linux 10 Security Technical Implementation Guide v1r1

### Current Profile Statistics

The profile will be tested on every commit and every release against both `vanilla` and `hardened` ubi and ec2 images using a CI/CD pipeline. The `vanilla` images are unmodified base images sourced from Red Hat itself. The `hardened` images have had their settings configured for security according to STIG guidance. Testing both vanilla and hardened configurations of both containerized and virtual machine implementations of RHEL10 is necessary to ensure the profile works in multiple environments.
...
The RHEL10.x STIG profile is `container aware` and is able to determine when the profile is being executed inside or outside a `docker container` and will only run the tests that are approporate for the enviroment it is testing in. The tests are all tagged as `host` or `host, container`.
...
```
inspec exec https://github.com/mitre/redhat-enterprise-linux-10-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/redhat-enterprise-linux-10-stig-baseline.git
inspec archive redhat-enterprise-linux-10-stig-baseline
...
```
cd redhat-enterprise-linux-10-stig-baseline
git pull
cd ..
inspec archive redhat-enterprise-linux-10-stig-baseline
```

control 'SV-281236' do
  ...
  desc 'check', 'Verify RHEL 10 is configured so that "/dev/shm" is mounted with the "noexec" option with the following command:

$ mount | grep /dev/shm
tmpfs on /dev/shm type tmpfs (rw,nodev,nosuid,noexec,seclabel)

If the "/dev/shm" file system is mounted without the "noexec" option, this is a finding.'
  desc 'fix', 'Configure RHEL 10 to mount "/dev/shm" with the "noexec" option.
